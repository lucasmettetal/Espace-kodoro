...
export async function onRequestPost({ request, env }) {
  ...
  if (test_email) {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: `${sender.name} <${sender.email}>`, reply_to: env.EMAIL_REPLY_TO ?? 'gaming.kodoro@gmail.com', to: [test_email], subject: `[TEST] ${subject}`, html }),
    });
    if (res.ok) {
  ...
  for (let i = 0; i < contacts.length; i += BATCH_SIZE) {
    ...
    const replyTo = env.EMAIL_REPLY_TO ?? 'gaming.kodoro@gmail.com';
