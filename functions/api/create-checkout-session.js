...
async function sendFirstVisitEmail(env, { to, customerName, quantity, event }) {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from, reply_to: env.EMAIL_REPLY_TO ?? 'gaming.kodoro@gmail.com', to: [to], subject: 'Ta première soirée Gaming est offerte ! — Espace Ködörö', html }),
    });
...
async function sendPassReservationEmail(env, { to, customerName, quantity, event, passExpiresAt }) {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${env.RESEND_API_KEY}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ from, reply_to: env.EMAIL_REPLY_TO ?? 'gaming.kodoro@gmail.com', to: [to], subject: 'Réservation confirmée — Soirée Gaming · Espace Ködörö', html }),
    });
    if (!res.ok) {
